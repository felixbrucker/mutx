...
#include "ui_zmutxcontroldialog.h"
...
std::list<std::string> zMutxControlDialog::listSelectedMints;
std::list<CZerocoinMint> zMutxControlDialog::listMints;

zMutxControlDialog::zMutxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::zMutxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
zMutxControlDialog::~zMutxControlDialog()
{
    delete ui;
}

void zMutxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zMutxControlDialog::updateList()
{
...
void zMutxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zMutxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelzMutx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzMutxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> zMutxControlDialog::GetSelectedMints()
{
...
void zMutxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
