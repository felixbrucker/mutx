                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMUTX ought to be enough for anybody." - Bill Gates, 2017
    ui->zMUTXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMUTXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMUTX->setEnabled(false);
        ui->pushButtonMintzMUTX->setToolTip(tr("zMUTX is currently disabled due to maintenance."));

        ui->pushButtonSpendzMUTX->setEnabled(false);
        ui->pushButtonSpendzMUTX->setToolTip(tr("zMUTX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMUTXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMUTX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMUTX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMUTX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMUTX in ") +
...
void PrivacyDialog::on_pushButtonSpendzMUTX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMUTX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMUTX
        sendzMUTX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMUTX
    sendzMUTX();
}

void PrivacyDialog::on_pushButtonzMutxControl_clicked()
{
    zMutxControlDialog* zMutxControl = new zMutxControlDialog(this);
    zMutxControl->setModel(walletModel);
    zMutxControl->exec();
}

void PrivacyDialog::setzMutxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMutxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMUTX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Mutx Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMUTXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMUTX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MUTX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMUTX</b>";
    ...
    // use mints from zMutx selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!zMutxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = zMutxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmutx selector in case it was used
    zMutxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMutx Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMUTX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMUTX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMUTX "));
    ui->labelzMUTXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMUTXSyncStatus->setVisible(fShow);
}
