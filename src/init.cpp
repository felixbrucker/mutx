...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MUTX and zMUTX money supply statistics") + " " + _("on startup"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, mutx, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Mutx specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzmutx=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMutx minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizemutxamount=<n>     " + strprintf(_("Keep N MUTX anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Mutex Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Mutex Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mutex Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Mutx: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Mutex Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Mutex Core to complete") << "\n";
        ...
        bool fEnablezMutxBackups = GetBoolArg("-backupzmutx", true);
        pwalletMain->setzMutxAutoBackups(fEnablezMutxBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeMutxAmount = GetArg("-anonymizemutxamount", 0);
//    if (nAnonymizeMutxAmount > 999999) nAnonymizeMutxAmount = 999999;
//    if (nAnonymizeMutxAmount < 2) nAnonymizeMutxAmount = 2;
    ...
    LogPrintf("Anonymize Mutx Amount %d\n", nAnonymizeMutxAmount);
    ...
    /* Denominations
       ...
       For example:
       1MUTX+1000 == (.1MUTX+100)*10
       10MUTX+10000 == (1MUTX+1000)*10
    */
